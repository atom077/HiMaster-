...
u8 client_init(void){
		u8 p[100];	
			sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s","192.168.1.188","8086");    //连接到制定pi服务器
...
//			///数据处理////////////////
			if(USART_RX_BUF[0]=='C'&&USART_RX_BUF[1]=='L'){  //检测是否CL命令
								if(USART_RX_BUF[3]=='M'&&USART_RX_BUF[4]=='2')      //检测是否控制的是M2
								{	
									 if(USART_RX_BUF[6]=='B'&&USART_RX_BUF[7]=='E'&&USART_RX_BUF[8]=='P'){ //BEEP 蜂鸣器控制代码块
												if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='N')	{		BEEP(1);	client_send("M2 BEEP is ON now!!!!\n");}  
												else if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='F'&&USART_RX_BUF[12]=='F')	{	BEEP(0);	client_send("M2 BEEP is OFF now!!!!\n");} 
										
										}
										else if(USART_RX_BUF[6]=='S'&&USART_RX_BUF[7]=='T'&&USART_RX_BUF[8]=='P'){ //STEP 步进电机 控制代码块
												if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='N')	
												{	if (!doorST){steprightoneround(); doorST=1;BEEP(ON); delay_ms(500);	BEEP(OFF) ;delay_ms(5);client_send("ST+M2+DOR+ON\n");}
													else {client_send("DOR already Open now ");}
												}  
												else if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='F'&&USART_RX_BUF[12]=='F')	{	
												if (doorST){stepleftoneround(); doorST=0;BEEP(ON); delay_ms(1000);	BEEP(OFF) ;	delay_ms(5);client_send("ST+M2+DOR+OFF\n");}
												else client_send("DOR already CLOSE now ");} 
			...
			if(doorST == 1)    //如果门开着  ， 延时5000秒 ，让自动门关闭  
			{delay_ms(5000);stepleftoneround(); doorST=0;BEEP(ON); delay_ms(1000);	BEEP(OFF) ;delay_ms(5);client_send("ST+M2+DOR+OFF\n");}
