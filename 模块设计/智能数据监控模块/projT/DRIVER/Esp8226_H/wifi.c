...
//			///数据处理////////////////
			if(USART_RX_BUF[0]=='C'&&USART_RX_BUF[1]=='L'){  //检测是否CL命令
								if(USART_RX_BUF[3]=='M'&&USART_RX_BUF[4]=='1')      //检测是否控制的是M1
								{	
										if (USART_RX_BUF[6]=='L'&&USART_RX_BUF[7]=='E'&&USART_RX_BUF[8]=='D'){//led控制代码块
												if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='N')	{		openled();	client_send("LED is ON now!!!!\n");}  
												else if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='F'&&USART_RX_BUF[12]=='F')	{	closeled();	client_send("LED is OFF now!!!!\n");} 

										}
										else if(USART_RX_BUF[6]=='B'&&USART_RX_BUF[7]=='E'&&USART_RX_BUF[8]=='P'){ //BEEP 蜂鸣器控制代码块
												if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='N')	{		BEEP(0);	client_send("BEEP is ON now!!!!\n");}  
												else if(USART_RX_BUF[10]=='O'&&USART_RX_BUF[11]=='F'&&USART_RX_BUF[12]=='F')	{	BEEP(1);	client_send("BEEP is OFF now!!!!\n");} 
