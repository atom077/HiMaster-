...
public class ModuleController {
    ...
    @ResponseBody
    public ServerResponse controlLed(String token) throws IOException, DemoException {
...
//			//填充业务
    ...
    @ResponseBody
    public ServerResponse controlDoor(String token) throws IOException, DemoException {
        if (token.equals(org.apache.commons.lang3.StringUtils.EMPTY)) {
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NONE_TOKEN.getCode(), "密钥不能为空！");
        }
        String phone = iModuleService.getPhone(token);
...
//			//填充业务
    ...
    @ResponseBody
    public ServerResponse controlSmartSocket(String token) throws IOException, DemoException {
        if (token.equals(org.apache.commons.lang3.StringUtils.EMPTY)) {
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NONE_TOKEN.getCode(), "密钥不能为空！");
        }
        String phone = iModuleService.getPhone(token);
